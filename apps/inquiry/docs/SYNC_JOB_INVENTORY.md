...
| `inquiry_mercari_daily_completeness_audit` | reconcile / internal_write / medium | Mercari Inquiry API → missing/changed delta only | `apps/worker/src/mercari/audit.ts`；daily `0 16 * * *` UTC, initial 72-hour lookback | `INQUIRY_COMPLETENESS_AUDIT_WRITES_ENABLED=true` | production active at `b6d7456`；two consecutive four-shop canaries wrote zero unchanged rows |
